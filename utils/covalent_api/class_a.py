...
class ClassA(object):
    ...
    def get_token_balances_for_address(
        ...
        #  ENS is.
        if chain_id not in list(AVAILABLE_CHAIN_IDS.values()):
            raise Exception(
                "chain_id should be one of {}".format(
                    list(AVAILABLE_CHAIN_IDS.values())
                )
            )
    ...
    def get_all_contract_metadata(
        ...
        '''
        if chain_id not in list(AVAILABLE_CHAIN_IDS_CONTRACT_METADATA.values()):
            raise Exception(
                "chain_id should be one of {}".format(
                    list(AVAILABLE_CHAIN_IDS_CONTRACT_METADATA.values())
                )
            )
        if id != 'all':
            raise Exception(
